...
- [~] **레벨 2 (부분 완료)**: 다중 셀러 시뮬. ✅ 카탈로그를 오퍼(셀러×모델) 구조로 전환, 같은 오픈모델을 셀러 여럿이 다른 가격에. 선택=최적 오퍼, 평판=셀러별. 라이브 데모: cheap→최저가 gamma→먹튀→평판 강등→정직한 beta 승리. ⬜ 남음: 셀러가 각자 프로세스(현재는 한 프록시가 다 대행), discovery 레지스트리, 셀러별 speed/uptime.
